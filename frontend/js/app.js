...
import {
  sel, data, maps, buildMaps, computeVisibleEvents,
  loadState, saveState, loadStateFromUrlParams, stateToUrlParams,
  getEventColor, TYPE_SHORT, STATUS_COLORS, STATUS_LABELS,
  WEEKDAY_LABELS, TYPE_COLORS,
} from './state.js';
import {
  fetchDegrees, fetchModules, fetchEvents, fetchStaff,
  fetchLocations, fetchSemesters, fetchDegreeDetail,
} from './api.js';
import { initCalendar, refreshCalendarEvents, changeCalendarView, setCalendarCallbacks } from './calendar.js';
import {
  renderFilters, setupSearchInputs, onDegreeChange,
  onSemesterChange, onFilterChange, resetAllFilters,
} from './filters.js';
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  try {
    ...
    const [degrees, modules, events, staff, locations, semesters] = await Promise.all([
      fetchDegrees(),
      fetchModules(),
      fetchEvents(),
      fetchStaff(),
...
function updateViewButtons(view) {
  document.querySelectorAll('[data-view]').forEach(btn => {
    btn.classList.toggle('active', btn.dataset.view === viewNameToShort(view));
  });
}
...
export function openEventPopup(eventId) {
  ...
  // Staff
  const staffNames = ev.staff_ids.map(sid => maps.staffById.get(sid)?.name).filter(Boolean);
  document.getElementById('popupStaff').textContent = staffNames.join(', ') || '–';

  // Status
  const statusDot = document.getElementById('popupStatusDot');
  statusDot.style.background = STATUS_COLORS[ev.status] || '#6b7280';
  document.getElementById('popupStatusLabel').textContent = STATUS_LABELS[ev.status] || ev.status;

  // Modules + Credits
  const modNames = ev.module_ids.map(mid => maps.moduleById.get(mid)?.name).filter(Boolean);
  const credits = ev.module_ids.map(mid => maps.moduleById.get(mid)?.credits).filter(Boolean);
  document.getElementById('popupModules').textContent = modNames.join(', ') || '–';
  document.getElementById('popupCredits').textContent = credits.length > 0 ? `${credits[0]} LP` : '–';
  ...
  degreesArea.parentElement.style.display = degreeTexts.length > 0 ? '' : 'none';

  // Pin button
  const pinBtn = document.getElementById('popupPinBtn');
  pinBtn.classList.toggle('pinned', isPinned);
  pinBtn.querySelector('.material-icons-round').textContent = isPinned ? 'push_pin' : 'push_pin';
  pinBtn.querySelector('.pin-label').textContent = isPinned ? 'Angeheftet' : 'Anheften';
  pinBtn.onclick = () => {
    onPinToggle(eventId);
  ...
  if (isPinned) {
    ...
    const siblings = data.events.filter(e =>
      e.id !== eventId &&
      e.type === ev.type &&
...
function updateColorModeUI() {
  ...
  const labels = { type: 'Typ', module: 'Modul', status: 'Status', staff: 'Dozent', custom: 'Custom' };
...
function renderCustomColorConfig() {
  ...
  for (const [typeName, defaultColor] of Object.entries(TYPE_COLORS)) {
    ...
    for (const color of SWATCH_COLORS) {
      ...
      const currentCustom = sel.customColors[`type:${typeName}`] || defaultColor;
...
function setupEventListeners() {
  ...
  document.getElementById('chipsResetBtn')?.addEventListener('click', resetAllFilters);
  ...
  document.getElementById('colorConfigCloseBtn')?.addEventListener('click', () => {
    document.getElementById('colorConfig')?.classList.remove('open');
  });

  // Expand toggles
  document.getElementById('weitereToggle')?.addEventListener('click', function () {
