...
export function setVisibleEventIds(ids) {
    visibleEventIds = ids;
}
export function setExcludedEventIds(ids) {
...
export function getModuleColor(moduleId) {
    if (!moduleColorCache.has(moduleId)) {
        moduleColorCache.set(
            moduleId,
            HUE_PALETTE[moduleColorCache.size % HUE_PALETTE.length],
        );
    }
...
export function getStaffColor(staffId) {
    if (!staffColorCache.has(staffId)) {
        staffColorCache.set(
            staffId,
            HUE_PALETTE[staffColorCache.size % HUE_PALETTE.length],
        );
    }
...
export function loadState() {
    try {
        ...
        if (Array.isArray(obj.pinnedEventIds))
            sel.pinnedEventIds = new Set(obj.pinnedEventIds);
...
export function stateToUrlParams() {
    ...
    if (sel.pinnedEventIds.size)
        params.set("pinned", [...sel.pinnedEventIds].join(","));

    const selMods = Object.entries(sel.modules)
        .filter(([, v]) => v === TRI.SELECTED)
        .map(([k]) => k);
    const hidMods = Object.entries(sel.modules)
        .filter(([, v]) => v === TRI.HIDDEN)
        .map(([k]) => k);
    if (selMods.length) params.set("mods_sel", selMods.join(","));
    if (hidMods.length) params.set("mods_hid", hidMods.join(","));

    const selTypes = Object.entries(sel.types)
        .filter(([, v]) => v === TRI.SELECTED)
        .map(([k]) => k);
    const hidTypes = Object.entries(sel.types)
        .filter(([, v]) => v === TRI.HIDDEN)
        .map(([k]) => k);
    if (selTypes.length) params.set("types_sel", selTypes.join(","));
    if (hidTypes.length) params.set("types_hid", hidTypes.join(","));

    const selStatus = Object.entries(sel.statuses)
        .filter(([, v]) => v === TRI.SELECTED)
        .map(([k]) => k);
    const hidStatus = Object.entries(sel.statuses)
        .filter(([, v]) => v === TRI.HIDDEN)
        .map(([k]) => k);
    if (selStatus.length) params.set("stat_sel", selStatus.join(","));
    if (hidStatus.length) params.set("stat_hid", hidStatus.join(","));

    const selStaff = Object.entries(sel.staffs)
        .filter(([, v]) => v === TRI.SELECTED)
        .map(([k]) => k);
    const hidStaff = Object.entries(sel.staffs)
        .filter(([, v]) => v === TRI.HIDDEN)
        .map(([k]) => k);
    if (selStaff.length) params.set("staff_sel", selStaff.join(","));
    if (hidStaff.length) params.set("staff_hid", hidStaff.join(","));

    const selLoc = Object.entries(sel.locations)
        .filter(([, v]) => v === TRI.SELECTED)
        .map(([k]) => k);
    const hidLoc = Object.entries(sel.locations)
        .filter(([, v]) => v === TRI.HIDDEN)
        .map(([k]) => k);
...
export function loadStateFromUrlParams() {
    ...
    if (!params.has("degree") && !params.has("pinned") && !params.has("sem"))
        return false;
    ...
    function loadTri(param, target) {
        if (params.has(param + "_sel")) {
            for (const k of params.get(param + "_sel").split(","))
                target[k] = TRI.SELECTED;
        }
        if (params.has(param + "_hid")) {
            for (const k of params.get(param + "_hid").split(","))
                target[k] = TRI.HIDDEN;
        }
    }
...
export function computeVisibleEvents() {
    ...
    for (const ev of allEvents) {
        ...
        if (
            hasDegreeFilter &&
            !ev.module_ids.some((mid) => allowedModuleIds.includes(mid))
        )
            continue;

        // Check all categories
        const passModule = passesCategory(ev, sel.modules, (e) =>
            e.module_ids.map(String),
        );
        const passType = passesCategory(ev, sel.types, (e) => [e.type]);
        const passStatus = passesCategory(ev, sel.statuses, (e) => [e.status]);
        const passStaff = passesCategory(ev, sel.staffs, (e) =>
            e.staff_ids.map(String),
        );
        const passLocation = passesCategory(ev, sel.locations, (e) => [
        ...
        switch (category) {
            ...
            case "staffs":
                for (const sid of ev.staff_ids) {
                    available.set(
                        String(sid),
                        (available.get(String(sid)) || 0) + 1,
                    );
                }
                break;
            case "locations":
                available.set(
                    String(ev.location_id),
                    (available.get(String(ev.location_id)) || 0) + 1,
                );
                break;
            case "modules":
...
function passesAllExcept(ev, exceptCategory) {
    ...
    if (exceptCategory !== "modules") {
        if (!passesCategory(sel.modules, (e) => e.module_ids.map(String)))
            return false;
    }
    ...
    if (exceptCategory !== "staffs") {
        if (!passesCategory(sel.staffs, (e) => e.staff_ids.map(String)))
            return false;
    }
    if (exceptCategory !== "locations") {
        if (!passesCategory(sel.locations, (e) => [String(e.location_id)]))
            return false;
    }
    return true;
}
